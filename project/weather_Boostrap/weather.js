...
fetch(
  ...
  .then(function (data) {
    ...
    let westArea = ["苗栗縣", "臺中市", "彰化縣", "南投縣", "雲林縣"];
    let southArea = [
      "嘉義縣",
      "嘉義市",
      "臺南市",
    ...
    data.records.location.forEach((city) => {
      ...
      //--- 比對資料並分類
      if (westArea.indexOf(cityName) >= 0) {
        cityBox.sortNumber = westArea.indexOf(cityName);//把索引值附值到排序依據
        allData.west.push(cityBox);
      } else if (southArea.indexOf(cityName) >= 0) {
        cityBox.sortNumber = southArea.indexOf(cityName);
        allData.south.push(cityBox);
      } else if (eastArea.indexOf(cityName) >= 0) {
        cityBox.sortNumber = eastArea.indexOf(cityName);
        allData.east.push(cityBox);
      } else if (outArea.indexOf(cityName) >= 0) {
        cityBox.sortNumber = outArea.indexOf(cityName);
        allData.outlying.push(cityBox);
      } else {
        cityBox.sortNumber = northArea.indexOf(cityName);
        allData.north.push(cityBox);
      }
      //對allData的資料做排序
      ["north", "west", "south", "east", "outlying"].forEach((item) => {
        allData[item].sort((a, b) => a.sortNumber - b.sortNumber);
      });

     
    });
    changeArea("north"); //顯示初始地區
  });
...
function changeArea(area) {
  let weatherContext = document.querySelector(`#${area}`).childNodes[1];//childNodes:選取子層
