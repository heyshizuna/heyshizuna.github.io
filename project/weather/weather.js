...
function changeArea(area) {
  ...
  allData[area].forEach((item) => {
    ...
    <div class="weathergroup">
                        <p class="city">${item.locationName}</p>
                        <div class="cardgroup">
                            <div class="card first">
                                <h4 class="when">今晚明晨</h4>
                                <p class="time">18:00 ~ 06:00</p>
                                <img class="wIcon" src="./IMG/icon/${item.icon[0]}.svg" alt="">
                                <p class="tc">${item.minTemperature[0]}° - ${item.maxTemperature[0]}°</p>
                                <p class="feel">${item.discription[0]}</p>
                                <p class="rain"><i class='bx bx-cloud-light-rain'></i>${item.rain[0]}%</p>
                            </div>
                            <div class="card second">
                                <h4 class="when">明日早上</h4>
                                <p class="time">06:00 ~ 18:00</p>
                                <img class="wIcon" src="./IMG/icon/${item.icon[1]}.svg" alt="">
                                <p class="tc">${item.minTemperature[1]}° - ${item.maxTemperature[1]}°</p>
                                <p class="feel">${item.discription[1]}</p>
                                <p class="rain"><i class='bx bx-cloud-light-rain'></i>${item.rain[1]}%</p>
                                </div>
                            <div class="card third">
                                <h4 class="when">明日晚上</h4>
                                <p class="time">18:00 ~ 06:00</p>
                                <img class="wIcon" src="./IMG/icon/${item.icon[2]}.svg" alt="">
                                <p class="tc">${item.minTemperature[2]}° - ${item.maxTemperature[2]}°</p>
                                <p class="feel">${item.discription[2]}</p>
                                <p class="rain"><i class='bx bx-cloud-light-rain'></i>${item.rain[2]}%</p>
                            </div>
                        </div>
                    </div>
    `;
